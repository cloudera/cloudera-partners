...
+
image::images/import_flow_registry.png[import flow registry, width=500, height=300]
...
- Double-click to open each flow.
- Locate the processor named InvokeHTTP (there is one per data source).
- Double-click the InvokeHTTP processor.
- Go to the Properties tab and find the HTTP URL field.
- Replace <your-server-ip> in the URL with the IP address or hostname of the server running your dummy generator app.
For example:
http://<your-server-ip>:8000/campaign-details
+
image::images/http_url.png[invoke http url]
+
Repeat this for each NiFi flow.
...
=== 8. Import Dashboard into DataViz

...
git clone --depth 1 --branch feature/k8soperators https://github.com/cloudera/cloudera-partners.git /tmp/repo && \
    ...
    cp -r /tmp/repo/K8SOperators/demo-resources ~/k8soperators/ && \
    rm -rf /tmp/repo
...
[source, bash]
----
minikube start --cpus 8 --memory 30000 --kubernetes-version=v1.32.0
----
...
[NOTE]
====
Make sure all the three instances of `<admin_password>` in `overrides/openldap-values.yaml` are updated with a secure password of your choice.  
...
[source, bash]
----
kubectl apply -f overrides/clusterissuer.yaml -n cert-manager
----
...
kubectl create secret generic openldap-creds \
  --from-literal=managerPassword=[***admin_password***] \
  -n demo-nifi
----
...
[source, bash]
----
ssh -f -N -i 8443:192.168.49.2:32156 -L ubuntu@13.215.183.137
----
+
[NOTE]
====
Since a self-signed certificate is used, you may need to accept the security exception in your browser.

URL: link:https://localhost:8443/nifi/[https://localhost:8443/nifi/]

Default user: `nifiadmin`

Password: `Same admin password as used for OpenLDAP chart and secret`
====

. Expose the NiFi UI when running minikube on `local laptop`
+
[source,bash]
----
...
URL: link:https://localhost:18443/nifi-registry/[https://localhost:18443/nifi-registry]
...
. Expose the `NiFi Registry` UI when running `minikube` on `local laptop`
+
[source,bash]
----
...
[NOTE]
====
URL: link:http://localhost:8081[http://localhost:8081]
====
...
[NOTE]
====
URL: link:http://localhost:18121[http://localhost:18121]
...
[source,bash]
----
kubectl -n csa-operator-system port-forward service/ssb-sse 18121:8081
----
